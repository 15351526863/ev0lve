...
void hsv_slider::render()
{
	...
	d->add_rect_filled_multicolor(
		saturation, {color::black_transparent(), color::black_transparent(), color::black(), color::black()});
	...
	d->add_rect(saturation.expand(1.f), colors.outline);

	// --- hue ---
	const std::vector<color> hue_parts = {color(255, 0, 0),	  color(255, 255, 0), color(0, 255, 0),
										  color(0, 255, 255), color(255, 0, 255), color(255, 0, 0)};

	d->add_shadow_rect(hue, 4.f);

	const auto part_height = hue.height() / float(hue_parts.size() - 1);
	for (auto i = 0; i < hue_parts.size() - 1; i++)
	{
		const auto &p = hue_parts[i];
		const auto &n = hue_parts[i + 1];

		d->add_rect_filled_multicolor(hue.translate({0.f, (float)i * part_height}).height(part_height), {p, p, n, n});
	}

	d->add_rect(hue.expand(1.f), colors.outline);

	const auto hue_grip_pos =
		hue.tl() + vec2{-2.f, std::clamp(((float)fake_hue / 360.f) * hue.height(), 0.f, hue.height() - 1)};
	...
	if (allow_alpha)
	{
		...
		d->add_rect(alpha.expand(1.f), colors.outline);

		const auto alpha_grip_pos =
			alpha.tl() + vec2{std::clamp(value->value.a * alpha.width(), 0.f, alpha.width() - 1), -2.f};
...
void hsv_slider::on_mouse_move(const vec2 &p, const vec2 &d)
{
	...
	case e_sb:
		...
		break;
	case e_hue:
		fake_hue = (uint16_t)std::clamp((p.y - hue.tl().y) / hue.height() * 360.f, 0.f, 360.f);
		break;
	case e_alpha:
