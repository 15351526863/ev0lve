...
void widget::reset() { pos = position.get(); }
...
void widget::render()
{
	...
	// render header
	const auto header_area = a.size({size.x, 24.f});
	...
	d->font = draw.fonts[GUI_HASH("gui_main")]; // gui_bold
...
void widget::toggle_visibility(bool vis) { alpha_anim.direct(vis ? 1.f : 0.f); }
} // namespace evo::gui