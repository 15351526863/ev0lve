...
void dialog_box::on_first_render_call()
{
	...
	message = fnt->wrap_text(message, 600.f);
	...
	const auto affirmative = std::make_shared<button>(control_id{id + 1, ""}, get_button_name(0));
	...
	const auto negative = std::make_shared<button>(control_id{id + 2}, get_button_name(1));
	...
	{
		const auto least = std::make_shared<button>(control_id{id + 3}, get_button_name(2));
...
void dialog_box::render()
{
	...
	d->add_text(area.center(), message, colors.text,
				text_params::with_vhline(align_center, align_center, align_center));
